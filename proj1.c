...
int ocupacao_aeroporto(char o[]){
	int k = 0, i = 0, ocupacao = 0;
	for(k = 0; k < contador_aeroportos; k++){
		if(strcmp(airport[k].id, o) == 0){
			for(i = 0; i < contador_aeroportos; i++){
				ocupacao += matriz[i][k] + matriz[k][i];
			}
			return ocupacao;
		}	
	}
 ...
 */
void altera_capacidade_maxima() { 
	int validade = 0, j = 0, capacidade_inserida = 0, colunas = 0, guarda_ocupacao = 0;
	char i[4] = "\0";
	validade = le_id(i);
	if(validade == 0){
		...
		for(j = 0; j < contador_aeroportos; j++){
			if((strcmp(i, airport[j].id) == 0)){
				if((airport[j].state == 1) && ((airport[j].capacity + capacidade_inserida) > 0)){	
						for(colunas = 0; colunas < contador_aeroportos; colunas++){
							guarda_ocupacao += matriz[j][colunas] + matriz[colunas][j];
						}
						if(guarda_ocupacao <= (airport[j].capacity + capacidade_inserida)){
				...
				for(v = 0; v < contador_aeroportos; v++){ 
					if((strcmp(airport[k].id, i) == 0) && (strcmp(airport[v].id, j) == 0) ){
						if(airport[k].state == 1 && airport[v].state == 1){
							if(test == 3){
								#if DEBUG
								printf("aeroporto com indice k, aeroporto com indice v: %s\t%s\n", airport[k].id, airport[v].id);
								#endif
...
void numero_voos(){
	int k = 0, v = 0;
	char i[4] = "\0", j[4] = "\0";
	if((le_id(i) == 0) && (le_id2(j) == 0)){/*verifica se a sequencia de letras introduzidas esta correta (verificação facultativa)*/
			...
			for(k = 0; k < contador_aeroportos; k++){
				if((strcmp(airport[k].id, i) == 0)){
					for(v = 0; v < contador_aeroportos; v++){
						...
						}/*este if nao falha, apenas serve para "salvar" os indices*/
					}/*END OF FOR*/ 
				}/*este if nao falha, apenas serve para "salvar" os indices*/
			}/*END OF FOR*/
		}
 ...
 */
void aeroporto_mais_voos(){
	int k = 0, indice = 0, l = 0, soma_temporaria = 0, soma_permanente = 0, ida_temporaria = 0, vinda_temporaria = 0, ida_permanente = 0, vinda_permanente = 0;
	for(k = 0, soma_temporaria = 0/*n tirar*/, ida_temporaria = 0, vinda_temporaria = 0; k < contador_aeroportos; k++, soma_temporaria = 0, ida_temporaria = 0, vinda_temporaria = 0){/*este for percorre as linhas*/
	...
	printf("Aeroporto com mais rotas %s:%d:%d\n", airport[indice].id, ida_permanente, vinda_permanente); 
}
...
void aeroporto_mais_conectado(){
	...
	for(linhas = 0, indice = 0, soma_temporaria = 0; linhas < contador_aeroportos; linhas++, soma_temporaria = 0){
		for(colunas = 0; colunas < contador_aeroportos; colunas++){
			if((matriz[linhas][colunas] > 0 && matriz[colunas][linhas] == 0) || (matriz[linhas][colunas] == 0 && matriz[colunas][linhas] > 0) || (matriz[linhas][colunas] > 0 && matriz[colunas][linhas] > 0)){ /*contabiliza para quantos lados diferentes ou de quantos lados diferentes sao efetuados os voos de/para cada um dos aeroportos*/
				soma_temporaria	+= 1; 
			}
		}
		#if DEBUG
		printf("soma temporaria do %s é %d\n", airport[linhas].id, soma_temporaria);
		#endif
		if(soma_temporaria > soma_permanente){
			soma_permanente = soma_temporaria;
			indice = linhas;
			#if DEBUG
			printf("Estou a assumir o indice: %d\n", indice);
			#endif
		}
	}		
